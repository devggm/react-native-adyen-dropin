   ...
   Log.d(TAG, "Called setDropInConfig")
    ...
    if (config == null) {
      ...
      try {
        ...
        if (config.hasKey("minAmount")) {
          val map = config.getMap("minAmount")!!
          memoryStorage.minAmount = RNUtils.convertMapToJson(map)
        }
        ...
        if (config.hasKey("amount")) {
          ...
          memoryStorage.shopperEmail = amount.getString("emailAddress")!!
        }
